          ...
          {milestones.map((milestone, index) => (
            <div key={index} className="flex flex-col md:flex-row items-center w-full md:w-auto">
              <div className="flex flex-col items-center text-center flex-1 w-full">
                <div 
                  className="w-full bg-secondary text-secondary-foreground rounded-lg flex flex-col justify-start items-center p-4 shadow-md flex-grow h-full"
                >
                  <div className="font-bold text-base mb-2">{milestone.label}</div>
                  <div className="font-semibold text-sm mb-3 break-words">{milestone.title}</div>
                  {milestone.tasks && (
              ...
              {index < milestones.length - 1 && (
                <ChevronRight className="w-8 h-8 text-muted-foreground mx-2 self-center shrink-0 hidden md:block" />
              )}
            </div>
          ))}
        </div>
